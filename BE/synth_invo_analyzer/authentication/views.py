...
from .permissions import IsEmployee, IsOrganization, IsSupplier, IsSystemAdmin
...
from .models import Employee, Organization, Supplier, SystemAdmin, SupplierOrganization, OTP, SupplierRequest
from .serializers import EmployeeSerializer, SupplierSerializer, SystemAdminSerializer, OrganizationSerializer, SupplierRequestSerializer, SupplierOrganizationSerializer
...
def add_employee(request):
    try:
        subject = 'Your Temperory Credentials For SynthInvoAnalyzer'
        ...
        "organization": request.data.get('orgId')
        }
    
        serializer = EmployeeSerializer(data=serializer_data)
        if serializer.is_valid():
            employee = serializer.save()
            send_email(request.data.get('email'),subject, 'employee_add_email.html', context)
            user = employee.user
...
def employee_protected(request):
    return Response({'message': 'Authenticated as employee'}, status=status.HTTP_200_OK)
